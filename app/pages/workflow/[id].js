...
export default function WorkflowPage() {
  ...
  const org = orgData?.organizations?.find(
    (o) => o.id === workflow?.org_id
  );

  const myRole = org?.org_members?.find(
    (m) => m.user_id === userId
  )?.role;
  ...
  async function handleAddTrigger(e) {
    ...
    try {
      ...
      console.error("Add trigger failed:", err);
      alert(err.message);
    }
  }
  ...
  async function handleRun() {
    try {
      ...
      setActiveRunId(
        response.data?.triggerWorkflowRun?.workflow_run_id
      );
    } catch (err) {
          ...
          <div>
            ...
            {(stepType === "db_write" ||
              ...
              myRole !== "owner" && (
                ...
                >
                  Only an org owner can add{" "}
                  {stepType} steps — this insert will
                  be rejected by Hasura permissions.
                </p>
              )}
          </div>
        </form>
        ...
        >
          ...
          {triggerType === "webhook" &&
            myRole !== "owner" && (
              ...
              >
                Only an org owner can add webhook
                triggers.
              </p>
            )}
        </form>
